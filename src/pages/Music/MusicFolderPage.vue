  ...
  <q-page>
      ...
      <div class="col-2 layout"   v-for="artist in artistList">
        <q-card>
          <q-card-media  overlay-position="top">
            <img src="./../../statics/Drawn_wallpapers.jpg" @click="getAlbum(artist)">
            <q-card-title>
              <q-input class="label"
                :readonly="true"
                :value="artist"
    ...
    <div class="row  justify-center" v-if="showAlbums">
      <div class="col-2 layout"  v-for="album in albumList">
          ...
          <q-card-media overlay-position="top">
              <img src="./../../statics/Drawn_wallpapers.jpg" @click="selectAlbum(currentArtist,album)">
          </q-card-media>
            </router-link>
...
//parse vs recalculate from store.. prefer memory usage rather than CPU load
  ...
  methods:{
    ...
    selectAlbum:function (artist,album) {
      this.$store.dispatch("musicStore/player/changeCurrentAlbum",{artist,album})
    },
...
<style scoped  lang="stylus">
  ...
  .header{
    border none
    background-image linear-gradient(to right, $evening,$stormy);
    height 40px
  }
  .albumImg{
    height inherit
